...
class DetectTrajAnomaly:
    def __init__(
        ...
        self.surface_change_cutoff_multiplier = (
    ...
    def has_surface_changed(self):
        ...
        adslab_connectivity = self._get_connectivity(
            self.final_atoms[surf_idx]
        )
    ...
    def is_adsorbate_intercalated(self):
        ...
        frozen_atoms_idx = [
            idx for idx, tag in enumerate(self.atoms_tag) if tag == 0
        ]
        final_connectivity = self._get_connectivity(
            self.final_atoms,
        )

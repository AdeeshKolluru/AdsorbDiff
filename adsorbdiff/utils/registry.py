...
# Copyright (c) Facebook, Inc. and its affiliates.
# Borrowed from https://github.com/facebookresearch/pythia/blob/master/pythia/utils/registry.py.
"""
...
def _get_absolute_mapping(name: str):
    ...
    # e.g., `adsorbdiff.tasks.base_task.BaseTask`
    # we can use importlib to get the module (e.g., `adsorbdiff.tasks.base_task`)
...
class Registry:
    ...
    def register_task(cls, name: str):
        ...
        Usage::
            from adsorbdiff.utils.registry import registry
            from adsorbdiff.tasks import BaseTask
    ...
    def register_dataset(cls, name: str):
        ...
        Usage::
            ...
            from adsorbdiff.utils.registry import registry
            from adsorbdiff.datasets import BaseDataset
    ...
    def register_model(cls, name: str):
        ...
        Usage::
            ...
            from adsorbdiff.utils.registry import registry
            from adsorbdiff.modules.layers import CGCNNConv
    ...
    def register_logger(cls, name: str):
        ...
        def wrap(func: Callable[..., R]) -> Callable[..., R]:
            from adsorbdiff.utils.logger import Logger
    ...
    def __import_error(cls, name: str, mapping_name: str) -> RuntimeError:
        ...
        if existing_cls_path is not None:
            ...
            existing_cls_path = "adsorbdiff.trainers.ForcesTrainer"
    ...
    def get_class(cls, name: str, mapping_name: str):
        ...
        # mapping be class path of type `{module_name}.{class_name}` (e.g., `adsorbdiff.trainers.ForcesTrainer`)
        if name.count(".") < 1:
